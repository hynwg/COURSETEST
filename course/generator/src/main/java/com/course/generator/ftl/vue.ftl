        ...
        <table id="simple-table" class="table  table-bordered table-hover">
            ...
            <tr v-for="${domain} in ${domain}List">
                ...
                <#list fieldList as field>
                    <#if field.nameHump!="createdAt" && field.nameHump!="updatedAt">
                        <#if field.enums>
                            <td>{ $filters.optionKV({${field.enumsConst},${domain}.${field.nameHump})}}</td>
                        <#else>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            query(page) {
                let _this = this;
                _this.$axios.post(process.env.VUE_APP_SERVER + '/${module}/admin/${domain}/query', {
                    page: page,
                    ...
                    _this.${domain}List = resp.content.list;
            ...
            save() {
                ...
                Loading.show();
                _this.$axios.post(process.env.VUE_APP_SERVER + '/${module}/admin/${domain}/save',
                    _this.${domain}
                ).then((response) => {
                    Loading.hide();
                    console.log("保存${tableNameCn}列表", response);
                    let resp = response.data;
                    if (resp.success) {
            ...
            del(id) {
                let _this = this;
                Confirm.show("确认删除${tableNameCn}后不可恢复", function () {
                    Loading.show();

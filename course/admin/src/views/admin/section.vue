...
export default {
  ...
  methods: {
    ...
    query(page) {
      let _this = this;
      _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/section/query', {
        page: page,
        size: _this.$refs.pagination.size,
      }).then((response) => {
        Loading.hide();
        console.log("查询小节列表", response);
        let resp = response.data;
        _this.sectionList = resp.content.list;
    ...
    save() {
      ...
      Loading.show();
      _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/section/save',
          _this.section
      ).then((response) => {
        Loading.hide();
        console.log("保存小节列表", response);
        let resp = response.data;
        if (resp.success) {
    ...
    del(id) {
      let _this = this;
      Confirm.show("确认删除小节后不可恢复", function () {
        Loading.show();

...
export default {
  ...
  methods: {
    ...
    all() {
      ...
      _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/category/all', {}).then((response) => {
        ...
        for (let i = 0; i < _this.categoryList.length; i++) {
          let c = _this.categoryList[i];
          if (c.parent === 0) {
            _this.level1.push(c);
            for (let j = 0; j < _this.categoryList.length; j++) {
              let child = _this.categoryList[j];
    ...
    save() {
      ...
      Loading.show();
      _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/category/save',
          _this.category
      ).then((response) => {
        Loading.hide();
        console.log("保存分类列表", response);
        let resp = response.data;
        if (resp.success) {

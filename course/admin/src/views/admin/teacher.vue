<template>
  <div>
    ...
    <div class="row">
      <div v-for="teacher in teacherList" class="col-md-3">
        <div>
          ...
          <div class="modal-body">
            <form class="form-horizontal">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <textarea v-model="teacher.intro" class="form-control"></textarea>
                </div>
              </div>
            </form>
          </div>
...
export default {
  ...
  methods: {
    ...
    query(page) {
      let _this = this;
      Loading.show();
      _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/teacher/query', {
        page: page,
        size: _this.$refs.pagination.size,
      }).then((response) => {
        Loading.hide();
        console.log("查询讲师列表", response);
        let resp = response.data;
        _this.teacherList = resp.content.list;
    ...
    save() {
      ...
      Loading.show();
      _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/teacher/save',
          _this.teacher
      ).then((response) => {
        Loading.hide();
        console.log("保存讲师列表", response);
        let resp = response.data;
        if (resp.success) {
